...
function dropDisc(x) {
	...
	if (((disc.value % 2) == 1) && check) {
		removeEvents();
		movein(x - 1);
		disc.innerHTML = "<h3 style='color:#d80000;'>Player 1's Turn</h3>";  
		disc.value++;
		...
		removeEvents();
		movein(x - 1);
		disc.innerHTML = "<h3 style='color:#0141c1;'>Player 2's Turn</h3>";
		disc.value++;
...
function fightAi(x) {
	...
	if (check) {
		...
		if (!getWinner()) {
			moveRandom();
			disc.innerHTML = "<h3>Player "+ pTurns +"'s Turn</h3>";
			getWinner();
		}
	}
	}
...
function TvsT() {
    if (board.getState() != -1) {
		var disc = document.getElementById("turns");
		if (board.getState() == 1) {
			disc.innerHTML = "<h3 style='color:#d80000;'>Game Over. "+ Ai1.name +"  Ai Won.</h3>";
			window.clearInterval(timer);
		}
		else if (board.getState() == 2) {
...
//1 is Random Ai, 2 is TreeBot Ai, 3 is newTreeBot Ai
function selectAi(ai1x, ai2x) {
	if (ai1x == 1) {
		Ai1 = new randobot();
	...
	if (ai2x == 1) {
		...
		Ai2 = new betterTreeBot();
	}
	
	timer = window.setInterval(battleAi, 1000);
}
...
function battleAi() {
	if (board.getState() != -1) {
		...
		if (board.getState() == 1) {
			...
			disc.innerHTML = "<h3 style='color:#0141c1'>Game Over. "+ Ai2.name +" 2 Ai Won.</h3>";
			window.clearInterval(timer);
		}
		else {
			disc.innerHTML = "<h3 style='color:#ffffff'>Game Over. Game Tied.</h3>";
	...
	var disc = document.getElementById("turns");
	disc.innerHTML = "<h3 style='color:#d80000;'>"+ Ai1.name +"'s 1 Turn</h3>";

	if (trackTurns == 1 ) {
		var game = board;
		while (!board.move(Ai1.m(game))) {}
		trackTurns = 2;
		disc.innerHTML = "<h3 style='color:#0141c1;'>"+ Ai2.name +"'s 2 Turn</h3>";
	}
	else if (trackTurns == 2) {
		var game = board;
		while (!board.move(Ai2.m(game))) {}
...
function RandomAivstreeBotAi(x) {
	if (x == 1) {
		Ai1 = new randobot();
		Ai2 = new treeBot();
	}
...
function addAi2Event() {
	for (var i = 0; i < 6; i++) {
		for (var j = 0; j < 7; j++) {
            var cellname = document.getElementById("cell" + i + j);
			if (j == 0) {
				cellname.onclick = function (e) { fightAi2(1);};
			}
			if (j == 1) {
				cellname.onclick = function (e) { fightAi2(2);};
			}
			if (j == 2) {
				cellname.onclick = function (e) { fightAi2(3);};
			}
			if (j == 3) {
				cellname.onclick = function (e) { fightAi2(4);};
			}
			if (j == 4) {
				cellname.onclick = function (e) { fightAi2(5);};
			}
			if (j == 5) {
				cellname.onclick = function (e) { fightAi2(6);};
			}
			if (j == 6) {
				cellname.onclick = function (e) { fightAi2(7);};
			}
		}
	}
}
